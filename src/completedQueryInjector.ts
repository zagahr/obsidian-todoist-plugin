import {
  ...
  import CompletedTodoistQuery from "./ui/CompletedTodoistQuery.svelte";
  import ErrorDisplay from "./ui/ErrorDisplay.svelte";
  import type SvelteComponentDev from "./ui/CompletedTodoistQuery.svelte";
  
  export default class CompletedInjectedQuery {
    ...
    injectQuery(pendingQuery: PendingQuery) {
      ...
      const child = new InjectedQuery(pendingQuery.target, (root: HTMLElement) => {
        if (query.isOk()) {
          return new CompletedTodoistQuery({
            target: root,
            props: {
              query: query.unwrap(),
              api: this.api,
